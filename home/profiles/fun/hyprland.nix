...
lib.mkIf (config.my.desktopProfile == "fun") {
  ...
  wayland.windowManager.hyprland.settings = lib.mkForce {
    ...
    bind = [
      ...
      "$mainMod, S, pseudo,"
      "$mainMod, K, exec, sh -c 'state=/tmp/hypr-alpha-toggle; if [ -f \"$state\" ]; then hyprctl dispatch setprop active alpha 1; rm -f \"$state\"; else hyprctl dispatch setprop active alpha 0.94; touch \"$state\"; fi'"
